...
- **`weekday` anchors a weekly task to a calendar week** (`weekday = "monday"`, validated to `frequency = "weekly"`). `_frequency_due()` is the single place the due time is computed — `_should_run()` gates on it and `format_next_run()` displays it. Anchored: midnight of the first such weekday strictly after the last success, so a Sunday run is due again on Monday and a Monday run not until the next Monday, however many RunAtLoad invocations land between. Unanchored: last success + threshold. Measured 2026-09-20: a Monday card-delivery task ran on a Sunday (RunAtLoad), which under the 6-day rule made it skip the Monday and re-fire the following Saturday. The backoff cap still uses `FREQUENCY_THRESHOLDS`; an env-var frequency override lands after validation, so `_frequency_due()` ignores the anchor unless the frequency is `weekly`.
